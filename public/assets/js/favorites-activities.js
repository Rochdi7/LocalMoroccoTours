(function($) {
    ...
    function loadFavorites() {
        let cookieName = `favorite_activities`;
        let favorites = getCookie(cookieName);
        if (favorites) {
            let ids = favorites.split(',');
            ids.forEach(id => {
                $(`.js-favorite-btn[data-type="activity"][data-id="${id}"]`).addClass('is-favorited');
            });
        }
    }
    ...
    $(document).ready(function () {
        console.log('Activities favorites loaded');
        loadFavorites();

        $(document).on('click', '.js-favorite-btn[data-type="activity"]', function (e) {
            e.preventDefault();
            e.stopPropagation();
            toggleFavorite($(this));
        });
