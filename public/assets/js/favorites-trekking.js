(function($) {
    ...
    function loadFavorites() {
        let cookieName = `favorite_trekkings`;
        let favorites = getCookie(cookieName);
        if (favorites) {
            let ids = favorites.split(',');
            ids.forEach(id => {
                $(`.js-favorite-btn[data-type="trekking"][data-id="${id}"]`).addClass('is-favorited');
            });
        }
    }
    ...
    $(document).ready(function () {
        console.log('Trekking favorites loaded');
        loadFavorites();

        $(document).on('click', '.js-favorite-btn[data-type="trekking"]', function (e) {
            e.preventDefault();
            e.stopPropagation();
            toggleFavorite($(this));
        });
