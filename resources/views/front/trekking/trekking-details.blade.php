         ...
         <div class="container">
             <div class="row y-gap-30 justify-between">
                 <div data-anim="slide-up delay-1" class="col-lg-8">
                    ...
                    {{ $i <= round($trekking->rating) ? 'text-yellow-2' : 'text-light-2' }}"></i>
                                                 @endfor
                                             </div>
                                             <span>
                                                 {{ number_format($trekking->rating, 1) }}
                         ...
                         <div class="col-12">
                             <div class="relative overflow-hidden js-section-slider" data-gap="10"
                                 ...
                                 <div class="swiper-wrapper">
                                     @forelse ($trekking->gallery ?? [] as $image)
                                         <div class="swiper-slide">
                                             <img src="{{ $image }}" alt="{{ $trekking->title }}"
                     ...
                     <div class="bg-white rounded-12 shadow-2 py-30 px-30 md:py-20 md:px-20 mt-30">
                         ...
                         <form action="{{ route('front.trekking.reserve', $trekking->slug) }}" method="POST">
                             <div class="row y-gap-30 contactForm pt-30">
                                 @csrf

                                 {{-- Hidden Tour ID --}}
                                 <input type="hidden" name="tour_id" value="{{ $trekking->id }}">
                             ...
                             <div class="pt-30" data-review-id="{{ $review->id }}">
                                 ...
                                 <div class="d-flex items-center mt-15">
                                     <div class="d-flex x-gap-5">
                                         @php $rating = floor($review->rating); @endphp
                                         @for ($i = 1; $i <= 5; $i++)
                                             <i
                                                 class="icon-star text-yellow-2 text-10 {{ $i <= $rating ? '' : 'opacity-30' }}"></i>
                                         @endfor
                                     </div>
                                         ...
                                         <div class="tourCard__content px-10 pt-10">
                                             ...
                                             <div class="tourCard__rating d-flex items-center text-13 mt-5">
                                                 ...
                                                 <span class="text-dark-1 ml-10">
                                                     {{ number_format($similarTrekking->rating ?? 4.8, 1) }}
                                                     ({{ $similarTrekking->booked_count }})
                                                 </span>
                                             </div>
